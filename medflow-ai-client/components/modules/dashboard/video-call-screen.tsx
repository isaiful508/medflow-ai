...
export function VideoCallScreen({
  ...
  return (
    <div className="grid h-[calc(100vh-7.5rem)] gap-4 xl:grid-cols-[1fr_240px]">
      <div className="overflow-hidden rounded-3xl border border-(--mc-border)">
        <div className="relative flex h-full min-h-[560px] flex-col items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.2),transparent_35%),linear-gradient(180deg,#0f172a_0%,#020617_100%)]">
          ...
          <p className="mt-4 text-lg font-medium text-(--mc-text-80)">Dr. Priya Kapoor</p>
          <p className="text-sm text-(--mc-text-30)">Neurologist · Connected</p>
          <div className="absolute top-5 right-5 rounded-full bg-(--mc-soft) px-3 py-1 text-xs text-(--mc-text-80)">
            <span className="mr-2 inline-block h-2 w-2 rounded-full bg-rose-500" />
            08:24
          </div>
          <div className="absolute right-5 bottom-28 rounded-2xl border border-(--mc-border) bg-(--mc-soft) p-3">
      ...
      <GlassCard className="flex min-h-0 flex-col overflow-hidden bg-(--mc-card)">
        <div className="border-b border-(--mc-border) px-4 py-4 text-sm font-medium text-(--mc-text-70)">
          In-call Chat
        </div>
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
          <p className="text-center text-xs text-(--mc-text-30)">Session started</p>
        ...
        <div className="flex items-center gap-2 border-t border-(--mc-border) px-4 py-3">
          <input
            placeholder="Message..."
            className="h-10 flex-1 rounded-full border border-(--mc-border) bg-(--mc-soft) px-4 text-sm outline-none placeholder:text-(--mc-text-30)"
          />
